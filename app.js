/* =============================================================
   ...
   Organização:
     1.  Estado Global
     2.  IndexedDB  — armazena MP3s sem limite de 5 MB
     3.  Persistência (LocalStorage) — apenas metadados leves
     4.  Navegação + renderPage
     5.  Renderização — Biblioteca
     6.  Renderização — Favoritas
     7.  Renderização — Playlists
     8.  Renderização — Detalhe da Playlist
     9.  Renderização — Histórico
    10.  Renderização — Estatísticas
    11.  Renderização — Importar / Exportar
    12.  Exportar / Importar
    13.  CRUD de Músicas
    14.  Helpers de Vídeo e Thumbnail
    15.  CRUD de Playlists
    16.  Drag & Drop (Playlist)
    17.  Tela de Karaokê
    18.  Teleprompter
    19.  Sidebar
    20.  Tema (Dark / Light)
    21.  Modais
    22.  Busca, Filtro e Ordenação
    23.  Utilitários
    24.  Toasts
...
let pendingThumbData    = null;  // base64 ou URL (thumbnail — pequeno, fica no LS)
let pendingMp3File      = null;  // File object — vai para a IndexedDB ao salvar
let pendingMp3ObjectUrl = null;  // URL.createObjectURL para preview
...
/* ─────────────────────────────────────────────────────────────
   2. INDEXEDDB — Armazenamento de arquivos MP3
   Por que IndexedDB e não LocalStorage?
   ▸ LocalStorage: limite ~5 MB total para TUDO (metadados + dados)
   ▸ IndexedDB:    suporta centenas de MB por origem, sem overhead
   Um MP3 de 5 MB em base64 ocupa ~7 MB — impossível no LS.
   Na IndexedDB guardamos o Blob nativo, direto e eficiente.
───────────────────────────────────────────────────────────── */

const IDB_NAME    = 'KaraokeLiveDB';
const IDB_VERSION = 1;
const IDB_STORE   = 'mp3files';  // chave: songId | valor: Blob

let _idb = null;

/** Abre (ou reutiliza) a conexão com a IndexedDB */
function openIDB() {
...
/** Salva um Blob de MP3 na IndexedDB */
async function idbSaveMp3(songId, blob) {
  const db = await openIDB();
  return new Promise((resolve, reject) => {
    const tx  = db.transaction(IDB_STORE, 'readwrite');
    const req = tx.objectStore(IDB_STORE).put(blob, songId);
    req.onsuccess = () => resolve();
    req.onerror   = e => reject(e.target.error);
  });
}

/** Recupera um Blob de MP3 da IndexedDB */
async function idbGetMp3(songId) {
  const db = await openIDB();
  return new Promise((resolve, reject) => {
...
/** Remove um MP3 da IndexedDB */
async function idbDeleteMp3(songId) {
  const db = await openIDB();
  return new Promise((resolve, reject) => {
    const tx  = db.transaction(IDB_STORE, 'readwrite');
    const req = tx.objectStore(IDB_STORE).delete(songId);
    req.onsuccess = () => resolve();
    req.onerror   = e => reject(e.target.error);
  });
}

/** Retorna todos os blobs (para exportação completa) */
async function idbGetAllMp3s() {
  const db = await openIDB();
  return new Promise((resolve, reject) => {
    const results = [];
    ...
    req.onsuccess = e => {
      ...
      if (cur) { results.push({ id: cur.key, blob: cur.value }); cur.continue(); }
...
/** Converte Blob → data-URL base64 (apenas para exportação JSON) */
...
/** Converte data-URL base64 → Blob (importação JSON) */
...
/* ─────────────────────────────────────────────────────────────
   3. PERSISTÊNCIA (LocalStorage) — somente metadados leves
   MP3s ficam EXCLUSIVAMENTE na IndexedDB.
   O campo temMp3 (boolean) é o único rastro no LocalStorage.
───────────────────────────────────────────────────────────── */

function save() {
  try {
    // Remove qualquer resíduo binário antes de serializar
    const songsClean = songs.map(({ arquivoMp3, ...rest }) => rest);
    localStorage.setItem('kl_songs',     JSON.stringify(songsClean));
    ...
    showToast('Erro ao salvar. Tente exportar seus dados.', 'error');
...
function autoBackup() {
  try {
    ...
    localStorage.setItem(
      'kl_backup_' + new Date().toDateString(),
      JSON.stringify({ songs: songsClean, playlists, settings })
    );
  } catch { /* ignora se cheio */ }
}
  ...
  return `
    <div class="music-card" data-id="${s.id}">
      ...
      <div class="card-actions">
        ...
        <button onclick="openEditModal('${s.id}')">✏️</button>
        <button onclick="openAddToPlaylist('${s.id}')">📋</button>
      </div>
    </div>`;
}
...
function renderEmpty() {
  ...
  return `
    <div class="empty-state">
      ...
      <p>${hasSearch ? 'Tente outro termo ou filtro.' : 'Adicione sua primeira música e comece a cantar!'}</p>
      ${!hasSearch ? `<button class="btn btn-primary" onclick="openAddModal()">＋ Adicionar primeira música</button>` : ''}
    </div>`;
}
...
function renderFavorites() {
  ...
  return `
    ...
    ${favs.length === 0
      ? `<div class="empty-state"><div class="empty-icon">🤍</div><h3>Nenhuma favorita ainda</h3><p>Clique no coração em qualquer música para salvar aqui.</p></div>`
...
function renderPlaylists() {
  return `
    ...
    ${playlists.length === 0
      ? `<div class="empty-state"><div class="empty-icon">📋</div><h3>Nenhuma playlist</h3><p>Crie playlists para organizar suas músicas favoritas.</p><button class="btn btn-primary" onclick="openPlaylistModal()">＋ Criar playlist</button></div>`
...
function renderPlaylistDetail() {
  ...
  const rows    = plSongs.map((s, i) => `
    <div class="playlist-song-row" draggable="true" data-song-id="${s.id}" data-idx="${i}">
      ...
      <button class="btn btn-danger btn-sm btn-icon" onclick="removeSongFromPlaylist('${pl.id}','${s.id}')">🗑</button>
    </div>`).join('');
  return `
  ...
  return `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
      ...
      <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-lg);padding:24px">
        ...
        <p style="font-size:13px;color:var(--text-muted);margin-bottom:16px">
          Restaura músicas, playlists e MP3s de um arquivo JSON exportado.
        </p>
    ...
    <div style="margin-top:20px;background:rgba(248,113,113,0.08);border:1px solid rgba(248,113,113,0.2);border-radius:var(--radius-lg);padding:24px">
      ...
      <p style="font-size:13px;color:var(--text-muted);margin-bottom:14px">Apaga todos os metadados e MP3s. Não pode ser desfeito.</p>
...
async function exportAll() {
  ...
  try {
    const mp3Entries = await idbGetAllMp3s();
    const mp3Map = {};
    for (const entry of mp3Entries) mp3Map[entry.id] = await blobToBase64(entry.blob);
    const songsExport = songs.map(s => ({ ...s, arquivoMp3: mp3Map[s.id] || '' }));
    const data = { version: 3, exportedAt: new Date().toISOString(), songs: songsExport, playlists, settings };
...
async function exportPlaylist(plId) {
  ...
  try {
    ...
    for (const id of pl.songs) {
      const s = songs.find(x => x.id === id);
      if (!s) continue;
      const blob = await idbGetMp3(s.id);
      songsExport.push({ ...s, arquivoMp3: blob ? await blobToBase64(blob) : '' });
    }
    downloadJSON(
      { version: 3, exportedAt: new Date().toISOString(), songs: songsExport, playlists: [pl], settings },
...
async function importFile(e) {
  ...
  reader.onload = async ev => {
    try {
      ...
      showToast('Importando, aguarde…', 'info');
      if (merge) {
        let added = 0, skipped = 0;
        for (const s of data.songs) {
          if (songs.find(x => x.id === s.id)) { skipped++; continue; }
          if (s.arquivoMp3) await idbSaveMp3(s.id, base64ToBlob(s.arquivoMp3));
          const { arquivoMp3, ...meta } = s;
          meta.temMp3 = !!s.arquivoMp3;
          songs.push(meta); added++;
        }
        (data.playlists||[]).forEach(pl => { if (!playlists.find(x => x.id===pl.id)) playlists.push(pl); });
        showToast(`${added} adicionadas, ${skipped} já existiam.`, 'success');
      } else {
        for (const s of songs) await idbDeleteMp3(s.id).catch(()=>{});
        songs = []; playlists = [];
        for (const s of data.songs) {
          if (s.arquivoMp3) await idbSaveMp3(s.id, base64ToBlob(s.arquivoMp3));
          const { arquivoMp3, ...meta } = s;
        ...
        showToast('Biblioteca substituída!', 'success');
      }
      save(); renderPage();
...
async function clearAll() {
  if (!confirm('Apagar TODOS os dados, incluindo MP3s? Esta ação não pode ser desfeita.')) return;
  for (const s of songs) await idbDeleteMp3(s.id).catch(()=>{});
...
function openAddModal() {
  pendingThumbData = null; pendingMp3File = null; pendingMp3ObjectUrl = null;
...
async function openEditModal(id) {
  ...
  pendingMp3File = null; pendingMp3ObjectUrl = null;
  ...
  document.getElementById('thumb-url').value               = '';
  if (s.temMp3) {
    document.getElementById('mp3-filename').textContent     = '✅ MP3 salvo (IndexedDB)';
    document.getElementById('mp3-status').textContent       = '';
...
async function saveSong() {
  ...
  // Salva MP3 na IndexedDB (não no LocalStorage)
  let temMp3 = existing ? !!existing.temMp3 : false;
  if (pendingMp3File) {
    try {
      await idbSaveMp3(id, pendingMp3File);
      temMp3 = true;
    } catch (err) {
      console.error('Erro ao salvar MP3:', err);
      showToast('Erro ao salvar o MP3.', 'error');
    }
  }
  ...
  const song = {
    ...
    dataCriacao:   existing ? existing.dataCriacao : new Date().toISOString(),
    plays:         existing ? (existing.plays||0) : 0,
    temMp3,
  };
...
async function removeMp3FromSong() {
  ...
  if (id) await idbDeleteMp3(id).catch(()=>{});
  const s = songs.find(x => x.id === id);
  if (s) s.temMp3 = false;
  pendingMp3File = null; pendingMp3ObjectUrl = null;
  document.getElementById('mp3-filename').textContent     = 'Nenhum arquivo';
...
function duplicateSong() {
  ...
  const dup = Object.assign({}, s, {
    id: genId(), titulo: s.titulo + ' (cópia)',
    dataCriacao: new Date().toISOString(), plays: 0, temMp3: false,
  });
  songs.push(dup);
  save(); closeModal('modal-music'); renderPage();
...
function handleThumbFile(e) {
  ...
  const r = new FileReader();
  r.onload = ev => { pendingThumbData = ev.target.result; setThumbPreview(ev.target.result); };
...
/**
 * Captura o File do MP3 sem converter para base64.
 * O blob vai direto para a IndexedDB quando o usuário salvar a música.
 * Isso elimina completamente o problema de "armazenamento cheio".
 */
function handleMp3File(e) {
  const file = e.target.files[0]; if (!file) return;
  pendingMp3File      = file;
  pendingMp3ObjectUrl = URL.createObjectURL(file);
  const mb = (file.size / 1024 / 1024).toFixed(2);
  document.getElementById('mp3-filename').textContent = file.name;
  document.getElementById('mp3-status').textContent   = `📦 ${mb} MB — será salvo ao clicar em Salvar`;
  document.getElementById('btn-remove-mp3').style.display = '';
}
...
/* ─────────────────────────────────────────────────────────────
   17. TELA DE KARAOKÊ
   MP3 obtido da IndexedDB via idbGetMp3() e reproduzido com
   URL.createObjectURL — nunca passa pelo LocalStorage.
───────────────────────────────────────────────────────────── */
...
async function openKaraoke(id) {
  ...
  // ── Abre a tela IMEDIATAMENTE (ainda dentro do gesto do usuário)
  // Isso é crítico: qualquer await ANTES daqui quebra o contexto
  // de interação e o navegador bloqueia o autoplay do áudio.
  ...
  if (s.videoPlayback) {
    ...
    // Se tem MP3 junto com o vídeo, injeta os controles no container persistente
    if (hasBoth) {
      wrap.style.position = 'relative';
      const persBar = document.getElementById('kara-persistent-audio');
      persBar.style.display = 'flex';
      persBar.innerHTML = [
        '<button id="btn-mute-video" onclick="toggleKaraokeVideoMute()" title="Mutar/desmutar v\xEDdeo"',
        ...
        '\uD83D\uDD0A V\xEDdeo</button>',
        '<audio id="kara-audio" controls style="flex:1;height:32px;accent-color:var(--accent);min-width:0"></audio>',
        '<button id="btn-mute-mp3" onclick="toggleKaraokeMp3Mute()" title="Mutar/desmutar MP3"',
        ' style="background:rgba(255,255,255,0.12);border:none;border-radius:8px;',
        'padding:4px 8px;cursor:pointer;font-size:15px;color:#fff;white-space:nowrap;flex-shrink:0">',
        '\uD83D\uDD0A MP3</button>'
      ].join('');
      _loadMp3IntoPlayer(s);
    }

  } else if (s.temMp3) {
    // ── Apenas MP3, sem vídeo de fundo ───────────────────────────────────────
    wrap.style.display = 'flex'; noVid.style.display = 'none';
    wrap.innerHTML = `
      <div style="flex:1;display:flex;align-items:center;justify-content:center;
                  color:var(--text-muted);font-size:14px;gap:10px">
        <span style="font-size:28px">⏳</span> Carregando áudio…
      </div>`;
    _loadMp3IntoPlayer(s);

  } else {
    // ── Sem vídeo e sem MP3 — mostra capa/ícone ──────────────────────────────
...
/**
 * Busca o MP3 na IndexedDB e injeta no #kara-audio.
 * Chamada de forma assíncrona DEPOIS que a tela já está aberta,
 * por isso .play() via JS funciona sem precisar do gesto original.
 */
async function _loadMp3IntoPlayer(s) {
  try {
    const blob = await idbGetMp3(s.id);
    const persBar = document.getElementById('kara-persistent-audio');
    const wrap    = document.getElementById('kara-video-wrap');
    ...
    if (!blob) {
      const msg = '<span style="font-size:13px;color:#f87171">&#x2753; MP3 n\xE3o encontrado \u2014 fa\xE7a o upload novamente.</span>';
      // Modo vídeo+MP3: substitui conteúdo da barra persistente
      if (persBar && persBar.style.display !== 'none') {
        persBar.innerHTML = msg;
      } else if (wrap) {
        wrap.innerHTML = `<div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;color:var(--text-muted)"><span style="font-size:48px">&#x2753;</span><p style="font-size:14px">MP3 n\xE3o encontrado na IndexedDB.</p></div>`;
      }
      return;
    }

    const objUrl = URL.createObjectURL(blob);
    let audioEl  = document.getElementById('kara-audio');

    // Modo MP3 puro: monta o layout com capa e usa o container persistente para o <audio>
    if (!audioEl) {
      // Exibe capa/ícone no wrap principal
      wrap.innerHTML = `
        <div style="flex:1;display:flex;flex-direction:column;align-items:center;
                    justify-content:center;background:var(--bg-secondary);gap:20px">
          ${s.thumbnail
            ? `<img src="${escHtml(s.thumbnail)}"
                    style="width:220px;height:220px;border-radius:20px;object-fit:cover;
                           box-shadow:0 8px 32px rgba(0,0,0,0.5)">`
            : `<div style="width:220px;height:220px;border-radius:20px;
                           background:linear-gradient(135deg,var(--accent),var(--accent2));
                           display:flex;align-items:center;justify-content:center;
                           font-size:80px;box-shadow:0 8px 32px var(--accent-glow)">&#x1F3B5;</div>`
          }
      ...
      // Injeta o <audio> no container persistente (visível também no modo apresentação)
      persBar.style.display = 'flex';
      persBar.innerHTML = `<audio id="kara-audio" controls style="flex:1;height:32px;accent-color:var(--accent);min-width:0"></audio>`;
      audioEl = document.getElementById('kara-audio');
    }

    audioEl._objUrl = objUrl;
    audioEl.src     = objUrl;
    // Não dá autoplay automático — o usuário inicia pelo botão ▶ do teleprompter
    audioEl.onended = () => URL.revokeObjectURL(objUrl);

  } catch (err) {
    console.error('Erro ao carregar MP3 da IDB:', err);
    const wrap = document.getElementById('kara-video-wrap');
    if (wrap) wrap.insertAdjacentHTML('beforeend',
      `<div style="position:absolute;bottom:8px;left:50%;transform:translateX(-50%);
                   color:var(--danger);font-size:13px;background:rgba(0,0,0,0.7);
                   padding:6px 12px;border-radius:8px">Erro ao carregar o MP3.</div>`);
  }
}

function closeKaraoke() {
  tpStop();
  // Revoga a ObjectURL do MP3 para liberar memória
  const audio = document.getElementById('kara-audio');
  if (audio && audio._objUrl) URL.revokeObjectURL(audio._objUrl);
  ...
  wrap.innerHTML = '';
  wrap.style.position = '';
  // Esconde e limpa o container persistente de áudio
  const persBar = document.getElementById('kara-persistent-audio');
...
function toggleKaraokeVideoMute() {
  ...
  if (video) {
    ...
    btn.textContent = (video.muted ? '\uD83D\uDD07' : '\uD83D\uDD0A') + ' V\xEDdeo';
    btn.style.opacity = video.muted ? '0.5' : '1';
  } else {
    // YouTube: sem acesso direto — avisa o usuário
...
function tpStart() {
  ...
  // Sincroniza com o MP3: se existir e estiver pausado, dispara o play
  const audio = document.getElementById('kara-audio');
...
function tpStop() {
  ...
  // Pausa o MP3 junto (só se não chegou ao fim — onended já revogou a URL)
  const audio = document.getElementById('kara-audio');
...
/* ─────────────────────────────────────────────────────────────
   27. INICIALIZAÇÃO
   A IndexedDB é aberta primeiro para garantir que está pronta
...
openIDB()
  ...
  .catch(err => {
    ...
    // Inicia sem IDB — MP3s não funcionarão, mas o resto sim
    load();
    applyTheme();
    navigate('library');
    showToast('Atenção: IndexedDB indisponível. MP3s não serão salvos.', 'error');
  });
